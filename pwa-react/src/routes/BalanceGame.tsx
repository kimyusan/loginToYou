...
import {
  ...
  ReRender,
  ChartContent,
} from "../styles/BalanceGame/BalanceGame";
  ...
  useEffect(() => {
    axiosAuth
      ...
      .then((res) => {
        ...
        setRate(
          Math.ceil(
            (res.data.fvote / (res.data.fvote + res.data.svote)) * 1000
          ) / 10
        );
        ...
        makeChart(
          (res.data.fvote / (res.data.fvote + res.data.svote)) * 100,
          theme.color.sub3
        );
      })
      .catch((error) => console.log(error));
    ...
    <div>
      ...
      <QuestionBox>
        <div style={{ display: "flex", alignItems: "end" }}>
          <div
            className="answer1"
            onClick={() => (saveChoice(q1), setMyChoice(q1))}
          >
            Q1. {q1}
          </div>
          {myChoice === q1 ? <img src={profileImage} alt="나의선택" /> : null}
          {yourChoice === q1 && myChoice !== "" ? (
            <img src={yourProfileImage} alt="상대의선택" />
          ) : null}
        </div>
        ...
        >
          ...
          {yourChoice === q2 && myChoice !== "" ? (
            <img src={yourProfileImage} alt="상대의선택" />
          ) : null}
          <div
            className="answer2"
            onClick={() => (saveChoice(q2), setMyChoice(q2))}
          >
            Q2. {q2}
          </div>
        </div>
      ...
      <OurTitle>
        ...
        <div className="content">
          {yourNickName ? yourNickName : yourName} 님의 선택
        </div>
      </OurTitle>
      ...
      <ChartContent>
        <div
          className="ct"
          style={{ background: theme.color.sub3, color: theme.color.point }}
        >
          {q1} ({Rate}%)
        </div>
        <div
          className="ct"
          style={{ background: theme.color.point, color: theme.color.sub3 }}
        >
          {q2} ({100 - Rate}%)
        </div>
      </ChartContent>
